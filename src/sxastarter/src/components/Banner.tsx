...
const Banner = ({ fields }: BannerProps) => {
  ...
  return (
    <section className="w-full flex items-center justify-center py-12  bg-neutral1">
      <div className="max-w-[1280px] w-full flex flex-col-reverse md:flex-row justify-between items-center gap-10">
        ...
        <div className="w-full md:w-[40%] px-6">
          <Text
            field={BannerTitle}
            tag="h1"
            className="text-6xl font-semibold text-gray9 mb-6 font-monteserrate leading-[48px]"
          />

          <RichText
            field={fields?.BannerDesc}
            tag="span"
            className="text-md text-gray7 text-justify tracking-normal font-monteserrate"
          />

          <Link
            field={BannerCTA}
            className="inline-block px-6 py-6 no-underline mt-12 font-monteserrate text-[16px] font-normal text-neutral1 bg-primary1 rounded-md hover:bg-gray6 hover:text-neutral1 transition duration-300"
          />
        </div>
        ...
        <div className="relative w-[100%] md:block md:w-[60%] md:px-6 px-0">
          {/* Banner Image */}
          <Image field={fields?.BannerImage} className=" md:h-[80vh] h-[60vh] object-cover" />
        </div>
      </div>
    </section>
  );
